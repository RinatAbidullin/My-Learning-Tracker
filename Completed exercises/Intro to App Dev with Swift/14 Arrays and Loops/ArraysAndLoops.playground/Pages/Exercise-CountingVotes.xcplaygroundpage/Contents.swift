...
//:
var countOfYesVotes = 0
var countOfNoVotes = 0
...
if countOfYesVotes > countOfNoVotes {
    print("Больше ЗА")
} else if countOfYesVotes < countOfNoVotes {
    print("Больше ПРОТИВ")
} else {
    print("Поровну")
}
...
func printResults(forIssue issue: String, withVotes votes: [Bool]) {
    ...
    print("\(issue) \(countOfYesVotes) yes, \(countOfNoVotes) no")
}

printResults(forIssue: "Should we change the mascot?", withVotes:shouldMascotChangeVotes)
printResults(forIssue: "Should we install coffee vending machine?", withVotes:shouldInstallCoffeeVendingMachineVotes)
